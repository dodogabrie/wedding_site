<template>
  <section class="relative min-h-screen py-20 px-4 pb-32 sm:pb-40">
    ...
    <div class="max-w-3xl mx-auto">
      <!-- Header -->
      <h2 ref="header" class="font-script text-forest text-4xl md:text-5xl lg:text-6xl text-center mb-16 opacity-0">
        Programma
      </h2>
      ...
      <div ref="timeline" class="relative">
        <!-- Vertical line -->
        <div class="absolute left-4 md:left-8 top-0 bottom-0 w-0.5 bg-forest/30"></div>
        ...
        <div class="space-y-6 md:space-y-8">
          ...
          >
            <!-- Left: Time + dot -->
            <div class="relative flex-shrink-0 w-24 sm:w-28 md:w-32">
              <div class="absolute left-4 md:left-8 top-1/2 -translate-y-1/2 -translate-x-1/2 w-3 h-3 bg-forest rounded-full z-10"></div>
              <span class="block text-forest font-serif text-2xl sm:text-2xl md:text-3xl font-medium pl-12 md:pl-16">
                {{ item.time }}
              </span>
            </div>
            ...
            <div class="flex-1">
              <h3 class="text-forest font-serif text-2xl sm:text-2xl md:text-3xl font-medium">
                {{ item.title }}
              </h3>
              <p v-if="item.description" class="text-forest/80 font-serif mt-1 text-lg sm:text-xl md:text-xl">
                {{ item.description }}
              </p>
            </div>
